...
16. **Node-level (kubelet/CRI-O) trust for the Zot registry — `additionalTrustedCA`.** The Zot TLS transition (see docs/PHASE_LOG.md, the Zot TLS phase) made Zot serve HTTPS with an OpenShift service-serving cert, and wired that CA into the in-cluster build-modelcar push and compliance-artifact-scan inspect at the Task level (`update-ca-trust` on the pod's `service-ca.crt`, with an `inject-cabundle` ConfigMap fallback). This does NOT cover the one consumer that pulls the Zot ModelCar image at the NODE level: `deploy-model`'s KServe/vLLM serving pods pull the modelcar through kubelet/CRI-O, and nodes do not trust the OpenShift service-serving-signer CA by default. Making that pull work requires cluster-wide registry trust via `image.config.openshift.io/cluster` → `spec.additionalTrustedCA` (a ConfigMap the image-config/MCO machinery distributes to every node), which has node rollout / brief-service-restart (possibly drain) implications — this is deliberately NOT folded into the Zot TLS change and needs its OWN dedicated design review, because it is a cluster-wide, node-reboot-class change, not a Task-script or per-namespace fix. Until it lands, a real HuggingFace-sourced `ModelRequest` will complete `build-modelcar` and `compliance-artifact-scan` over trusted TLS but halt at `deploy-model`'s image pull. Note this is a trust question, NOT a `ImageDigestMirrorSet`/`ImageTagMirrorSet` question (those rewrite registry targets; they do not add CA trust).
