...
10. **Backlog note added during `docs/REVIEW_RESPONSE_PLAN.md` Phase 8's implementation (secret handling hardening), deliberately not built this pass**: the EvalHub bearer token generated for the `security-scan`/`promote-and-benchmark` Tasks' pipeline identity is currently minted via a per-`ModelRequest` `TokenRequest` and persisted into an owned, ephemeral `Secret` (`<ModelRequest>-evalhub-token`) so it can flow to Tekton by Secret reference like every other credential — this needed a narrowly-scoped `secrets: create;update;patch` RBAC grant on the core reconciler (see `modelrequest_controller.go`'s RBAC marker comment). A cleaner long-term alternative: have the `security-scan` Task read its own `pipeline` ServiceAccount's automatically-projected token (`/var/run/secrets/kubernetes.io/serviceaccount/token`, mounted into every pod running as that SA by default) directly, eliminating the `TokenRequest`/ephemeral-`Secret`/RBAC-grant machinery entirely for this one credential. Not built this phase because it requires editing the `security-scan` Task's own script to source the token from a file instead of an env var (a Task-*behavior* change, not just plumbing) — larger blast radius than this phase's scope, better suited to a dedicated follow-up once the `secretKeyRef`-by-name pattern this phase introduced has proven out in practice.
...
16. **Node-level (kubelet/CRI-O) pull of the Zot ModelCar image — CORRECTED, RESOLVED via reencrypt Route.** <del>This was originally framed as a *trust* question requiring cluster-wide `image.config.openshift.io/cluster` → `spec.additionalTrustedCA` (a ConfigMap distributed to every node with rollout/restart implications): `deploy-model`'s KServe/vLLM serving pods pull the modelcar through kubelet/CRI-O, and nodes do not trust the OpenShift service-serving-signer CA by default.</del> **That characterization was wrong.** The actual blocker discovered (see docs/PHASE_LOG.md, "Node-level Zot pull fix") is **DNS, not trust**: kubelet pulls images in the node's *host* network namespace using the node's OS resolver, which by design cannot resolve `.svc.cluster.local` at all — the kubelet error is `lookup zot.modelops-zot.svc.cluster.local on <node-resolver>: no such host`, and it happens *before any TLS handshake* (so `additionalTrustedCA` alone would never have fixed it). `ImageDigestMirrorSet`/`ImageTagMirrorSet` are likewise irrelevant (they rewrite targets; this is neither a mirroring nor a CA-trust problem). **Resolution:** a reencrypt `Route` for Zot (auto-generated `*.apps.<domain>` hostname, no committed cluster-specific literal) plus a `build-modelcar` change to emit the *Route hostname* as `image-ref`. Because the `*.apps` wildcard cert on this cluster is publicly trusted (ZeroSSL), the node both resolves and trusts the pull with zero node-rollout change — the reencrypt path sidesteps the would-be `additionalTrustedCA` work rather than performing it. Verified: ImagePullBackOff cleared, predictor pod `3/3 Running`, InferenceService `Ready: True`. **Remaining real cost:** Zot's registry/UI is re-exposed on the public `*.apps` zone; anonymous pull remains open. The deliberate follow-up (not done) is revoking anonymous pull + giving the serving pods pull auth (imagePullSecret), which needs either a `deploy-model-task` image rebuild or namespace ServiceAccount-level pull secrets.

15. **Real secrets management before leaving sandbox use.** `gitops/components/runtime-config/secrets.yaml` currently commits MinIO's well-known default credentials (`minioadmin`/`minioadmin`) as plaintext Secrets — intentional and acceptable only because this is a disposable sandbox cluster using a disposable in-cluster MinIO instance with no real data. Before this project moves to any non-sandbox deployment (demo, staging, production), introduce a real secrets-management pattern: either Sealed Secrets (bitnami-labs/sealed-secrets) for a GitOps-native encrypted-at-rest approach, or an External Secrets Operator (external-secrets.io) to sync from a vault/cloud-provider secret manager. The concrete motivating example is `scan-s3-credentials` and `result-s3-credentials` in `gitops/components/runtime-config/secrets.yaml` — they must not be committed as plaintext outside a disposable sandbox. Not scope for any current phase; needs its own design review before any deployment beyond sandbox.
