...
## Hardcoded cluster-hostname audit — remove/derive every cluster-specific domain literal

Not a numbered phase in `REFACTOR_PLAN.md`; a targeted audit driven by the
MinIO-endpoint-fix class of bug (a cluster-specific hostname committed in
`gitops/` that silently breaks on any differently-named cluster).

### Audit result (exhaustive)

Grep across `gitops/` + `operator/config/samples/` for cluster-specific
hostname literals (`.opentlc.com`, `*.apps.<cluster-domain>`) found exactly
five real literals plus one stale comment — the list raised in the task was
complete, nothing more was hidden:

1. `gitops/components/minio/deployment.yaml` — `MINIO_BROWSER_REDIRECT_URL`.
2. `gitops/components/maas/gateway.yaml` — two Gateway listener `hostname`s.
3. `gitops/components/maas/gateway-route.yaml` — Route `spec.host`.
4. `operator/config/samples/platformconfig-sample.yaml` — `evalhubUrl` (a
   Route address on a *different* cluster, `tkjpd`, than the other files'
   `mb9bl` — the classic copy-paste drift this audit targets).
5. `gitops/components/runtime-config/secrets.yaml` — a comment only (stale
   reference to yet another cluster's route domain).

Nothing depended on a *stable, predictable* name for the MinIO console Route
(auto-generated by OpenShift, consumed only by a browser login redirect), so it
did not need an explicit host. The MaaS `maas.apps.<domain>` hostname is a
genuine API requirement (RHOAI 3.5's maas-api reads `spec.listeners[].hostname`
to build its external URL; the passthrough Route must match the Gateway's host)
and therefore a derived value, not a removable one.

### What changed

- **MinIO (`deployment.yaml`)**: `MINIO_BROWSER_REDIRECT_URL` removed. It is
  only consumed during OpenID/IDP login and this deployment has no IDP
  (root user/password only), so the variable was inert; omitting it avoids
  duplicating the auto-generated console Route hostname. Comment documents that
  an IDP must derive the redirect from the Route at pod startup, never hardcode.
- **MaaS (`gateway.yaml`, `gateway-route.yaml`)**: the two listener `hostname`s
  and the Route `host` are now empty placeholders overwritten at build time by
  a kustomize `replacement`, sourced from one new ConfigMap
  `gitops/components/maas/cluster-config.yaml` (`data.maas-hostname`). Selection
  was made explicitly via the review question: the CM-Plugin option (fully
  auto-derived at sync) was declined in favor of the kustomize-replacement +
  single committed ConfigMap approach.
- **Sample `platformconfig-sample.yaml`**: `evalhubUrl` changed to the
  cluster-portable internal Service DNS (`https://default-evalhub.
  redhat-ods-applications.svc.cluster.local:8443`), matching the deployed
  `gitops/components/runtime-config/platformconfig.yaml` (the Phase 9 follow-up
  already made this exact change for the real Secret; the sample had missed it).
- **`secrets.yaml`**: stale cluster-domain comment removed.
- **Static safety net**: `operator/internal/stages/tekton/cluster_domain_test.go`
  (same pattern as the pipeline-YAML credential/CA/TLS tests) scans every
  `*.yaml`/`*.yml` under `gitops/` and `operator/config/samples/` and fails on
  any `.opentlc.com` literal or any `*.apps.<cluster-domain>` wildcard hostname,
  except the single allowlisted source (`cluster-config.yaml`) and generic
  placeholders (`*.example.com`, `*.your-cluster.com`). Verified it fails when
  a regression probe re-introduces an `.opentlc.com` literal.

### Verification

- `go build ./...`, `go vet ./...`, `go test ./...` all clean (the static test
  plus the full envtest-backed suite).
- `kustomize build gitops/components/maas` renders the Gateway listeners and
  Route host to `maas.apps.ocp.mb9bl.sandbox1545.opentlc.com` — byte-identical
  to what is live on the sandbox, so the derived values function exactly as
  before (confirmed no functional drift via `oc diff`, only the additive new
  ConfigMap and pre-existing ArgoCD tracking-id annotations). Live MaaS health
  `curl -sk https://maas.<domain>/maas-api/health` → `{"status":"healthy"}`.
- **Fresh-deploy proof**: editing *only* `cluster-config.yaml` to a different
  domain and rebuilding propagates the new hostname into both listener
  `hostname`s and the Route `host` with zero other edits — the regression this
  fix exists to prevent.

### Known limitation / follow-up

- This is **not** fully cluster-agnostic: on a new cluster the single value in
  `gitops/components/maas/cluster-config.yaml` must be set once (bootstrap
  checklist updated to say so). Making it fully automatic would require an
  ArgoCD Config Management Plugin (declined in favor of the simpler,
  review-visible single-edit-point approach); tracked here as a deliberate,
  documented tradeoff, not an oversight.
- MinIO's `MINIO_BROWSER_REDIRECT_URL` remains intentionally absent; if an
  OpenID IDP is ever added, the redirect must be derived from the live
  `minio-console` Route at pod startup (or the equivalent), never hardcoded.
