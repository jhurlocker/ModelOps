    ...
    for name, ns, deploy_name in all_checks:
        try:
            ...
            operators.append({
                ...
                "message": "Ready" if healthy else "Replicas not ready",
            })
        except Exception as exc:
            status = getattr(exc, "status", None)
            if status == 403:
                state, message = "Unavailable", "Service account cannot read this deployment"
            elif status == 404:
                state, message = "Not installed", "Deployment not present in this cluster"
            else:
                state, message = "Not installed", str(exc).split("\n")[0][:120] or "Deployment not present"
            operators.append({
                "component": name,
                "namespace": ns,
                "available": "0/0",
