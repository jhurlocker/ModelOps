    ...
    wizard.addEventListener('click', (e) => {
        ...
        if (nextBtn) {
            if (currentStep < steps.length - 1) {
                ...
                if (typeof window.updateIntakeReview === 'function') {
                    window.updateIntakeReview();
                }
            }
            return;
        }
...
function addPromotionNamespace() {
    ...
    if (!container) return;
    let max = -1;
    document.querySelectorAll('[name^="promotion-namespace-"]').forEach((node) => {
        const n = parseInt(node.name.split('-').pop(), 10);
