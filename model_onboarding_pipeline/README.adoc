...
== The Pipeline Flow (Conceptual — see operator/ for the current CRD-driven implementation)

This section describes the conceptual pipeline stages for understanding what the
onboarding process does. The actual production implementation is CRD-driven and managed
by the ModelOps operator's `ModelLifecycleProfile.Spec.Stages` (ordered stage list, per
docs/REFACTOR_PLAN.md Phase 6). The Tekton pipeline manifests referenced below
(`model-intake-sandbox`, `model-intake-promotion`) are execution-level details resolved
through `IntakeProviderConfig`, not hardcoded in the controller.

=== Stage: Sandbox governance (conceptual, automated)
...
=== Stage: Promote to staging (conceptual, human-gated)
...
=== Stage: MaaS production deployment (conceptual, optional)
...
==== 8. Deploy the pipelines and supporting infrastructure

The operator and its stage walker (see `operator/`) resolve pipeline names from
`ModelLifecycleProfile` and `IntakeProviderConfig` CRDs. The individual Tekton task
YAMLs deployed above are still used as the building blocks; the monolithic
`model-intake-pipeline.yaml` is no longer directly invoked by the current architecture.

For GitOps-based deployment, see `gitops/` and the root `README.md`.
...
== Running the Pipeline (current CRD-driven approach)

The production workflow is CRD-driven: submit a `ModelRequest` CR (via the Model
Intake UI or `kubectl`), and the operator handles the full lifecycle through the
stage walker defined in `ModelLifecycleProfile.Spec.Stages`. See the root
`README.md` and `operator/` for the current architecture.

```bash
oc apply -f operator/config/samples/modelrequest-sample.yaml
# Or submit via the Model Intake UI (deployed in the sandbox namespace)
```
